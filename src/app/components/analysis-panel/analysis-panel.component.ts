...
{
  @ViewChild("networkWithLegend", { static: false })
  networkWithLegendEl: ElementRef;
  ...
  @Output() visibleItems = new EventEmitter<
    [any[], [Node[], Tissue], NodeInteraction[]]
  >();
  ...
  private setNetworkListeners() {
    this.networkHandler.activeNetwork.networkInternal.on(
      "dragEnd",
      (properties) => {
        const node_ids =
          this.networkHandler.activeNetwork.networkInternal.getSelectedNodes();
    ...
    this.analysis.subscribeList((items, selected) => {
      ...
      if (selected !== null) {
        ...
        for (const item of items) {
          ...
          const pos =
            this.networkHandler.activeNetwork.networkInternal.getPositions([
              item.id,
            ]);
          node.x = pos[item.id].x;
          node.y = pos[item.id].y;
          const isSeed = this.networkHandler.activeNetwork.highlightSeeds
            ? this.networkHandler.activeNetwork.seedMap[node.id]
            : false;
        ...
        for (const item of items) {
          // TODO: Refactor!
          const found = proteinSelection.findIndex(
            (i) => getProteinNodeId(i) === item.id
          );
          const tableItem = this.tableProteins.find(
            (i) => getProteinNodeId(i) === item.id
          );
  ...
  private rankTable(
    table: Array<Drug & Scored> | Array<Node & Scored & Seeded>
  ) {
    let lastRank = 1;
    ...
    this.getView(this.token).then(async (view) => {
      ...
      return new Promise<any>(async (resolve, reject) => {
        ...
        if (this.drugstoneConfig.config.autofillEdges && nodes.length) {
          const node_map = {};
          nodes
            .filter((n) => n.drugstoneType === "protein")
            .forEach((node) => {
          ...
          const netexEdges = await this.netex.fetchEdges(
            nodes,
            this.drugstoneConfig.config.interactionProteinProtein,
            this.drugstoneConfig.config.licensedDatasets
          );
          edges.push(
            ...netexEdges
              .map((netexEdge) =>
        ...
        edges = edges.filter((edge) => {
          if (
            edge_map[edge.to] &&
            edge_map[edge.to].indexOf(edge.from) !== -1
          ) {
            return false;
          }
          if (
            edge_map[edge.from] &&
            edge_map[edge.from].indexOf(edge.to) !== -1
          ) {
            return false;
          }
        ...
        this.networkHandler.activeNetwork.inputNetwork = {
          nodes: nodes,
          edges: edges,
        };
        ...
        const container =
          this.networkHandler.activeNetwork.networkEl.nativeElement;
        const isBig = nodes.length > 100 || edges.length > 100;
        const options = NetworkSettings.getOptions(
          isBig ? "analysis-big" : "analysis",
          this.drugstoneConfig.currentConfig()
        );
        // @ts-ignore
        ...
        this.networkHandler.activeNetwork.networkInternal = new vis.Network(
          container,
          this.nodeData,
          options
        );
        ...
        this.networkHandler.activeNetwork.networkInternal.once(
          "stabilizationIterationsDone",
          async () => {
            if (
              !this.drugstoneConfig.config.physicsOn ||
              this.networkHandler.activeNetwork.isBig()
            ) {
              this.networkHandler.activeNetwork.updatePhysicsEnabled(false);
            }
            this.networkHandler
              .updateAdjacentNodes(this.networkHandler.activeNetwork.isBig())
              .then(() => {
                resolve(nodes);
              });
          }
  ...
  private async refreshTask() {
    ...
    if (this.task.info.algorithm === "degree") {
      ...
      this.task.info.algorithm === "closeness" ||
      this.task.info.algorithm === "quick" ||
      this.task.info.algorithm === "super"
    ) {
      this.tableDrugScoreTooltip =
      ...
      this.netex.getTaskResult(this.token).then(async (result) => {
        ...
        await this.createNetwork(this.result).then((nw) => {
          return new Promise<any>((resolve, reject) => {
            ...
            analysisNetwork.networkInternal = new vis.Network(
              container,
              this.nodeData,
              options
            );
            ...
            analysisNetwork.networkInternal.once(
              "stabilizationIterationsDone",
              async () => {
                if (
                  !this.drugstoneConfig.config.physicsOn ||
                  analysisNetwork.isBig()
                ) {
                  analysisNetwork.updatePhysicsEnabled(false);
                }
                this.networkHandler
                  .updateAdjacentNodes(analysisNetwork.isBig())
                  .then(() => {
                    resolve(nodes);
                  });
              }
            ...
            .then((nodes) => {
              this.tableDrugs = nodes.filter(
                (e) => e.drugstoneId && e.drugstoneType === "drug"
              );
              ...
              this.tableProteins = nodes.filter(
                (e) => e.drugstoneId && e.drugstoneType === "protein"
              );
              ...
              analysisNetwork.networkInternal.setData({
                nodes: undefined,
                edge: undefined,
              });
              setTimeout(() => {
            ...
            .then(() => {
              if (
                !["quick", "super", "connect", "connectSelected"].includes(
                  this.task.info.algorithm
                )
              ) {
                return;
              }
              this.netex
                .getAlgorithmDefaults(this.task.info.algorithm)
                .then((response) => {
  ...
  public emitVisibleItems(on: boolean) {
    if (on) {
      this.visibleItems.emit([
        this.nodeData.nodes,
        [this.proteins, this.selectedTissue],
        this.nodeData.edges,
      ]);
    } else {
  ...
  private async getView(token: string): Promise<any> {
    return await this.http
      .get(`${this.netex.getBackend()}view/?token=${token}`)
      .toPromise();
  }

  private async getTask(token: string): Promise<any> {
    return await this.http
      .get(`${this.netex.getBackend()}task/?token=${token}`)
      .toPromise();
  }
  ...
  close() {
    ...
    this.legendService.networkHasConnector = false;
  ...
  public async createNetwork(
    ...
    // @ts-ignore
      .filter((e) => e[1].drugstoneType === "protein")
      .forEach((e) => {
        // @ts-ignore
    ...
    for (const nodeId of network.nodes) {
      if (details[nodeId]) {
        ...
        nodeDetails.id = nodeDetails.id
          ? nodeDetails.id
          : typeof nodeDetails.drugstoneId === "string"
          ? nodeDetails.drugstoneId
          : nodeDetails.drugstoneId[0];
        if (
          nodeDetails.drugstoneId &&
          nodeDetails.drugstoneType === "protein"
        ) {
          ...
          nodeDetails.group =
            result.targetNodes && result.targetNodes.indexOf(nodeId) !== -1
              ? "foundNode"
              : nodeDetails.group
              ? nodeDetails.group
              : "connectorNode";
          nodeDetails.label = nodeDetails.label
            ? nodeDetails.label
            : nodeDetails[identifier];
          nodeDetails.id = nodeDetails[identifier][0]
            ? nodeDetails[identifier][0]
            : nodeDetails.id;
          this.proteins.push(nodeDetails);
        } else if (
          nodeDetails.drugstoneId &&
          nodeDetails.drugstoneType === "drug"
        ) {
    ...
    for (const edge of network.edges) {
      const e = mapCustomEdge(
        edge,
        this.drugstoneConfig.currentConfig(),
        this.drugstoneConfig
      );
      e.from =
        e.from[0] === "p" && nodeIdMap[e.from] ? nodeIdMap[e.from] : e.from;
    ...
    this.legendService.networkHasConnector =
      nodes.filter((node) => node.group === "connectorNode").length > 0;
